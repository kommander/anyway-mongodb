...
Store.prototype.addField = function(modelName, fieldName, options) {
  ...
  if(options.unique === true) {
    this.q.push(function(t, modelName, fieldName, next) {
      t._db.ensureIndex(modelName, fieldName, {
        unique: true, 
        background: true, 
        dropDups: true, 
        w: 1
...
Store.prototype.saveDocument = function(doc, cb) {
  this.q.push(this._qSaveDocument, [doc, cb], this);  
};

Store.prototype._qSaveDocument = function(doc, cb, next) {
...
Store.prototype.get = function(type, id, cb) {
  this.q.push(this._qGet, [type, id, cb], this);  
};

Store.prototype._qGet = function(type, id, cb, next) {
...
Store.prototype.find = function(type, query, options, cb) {
  this.q.push(this._qFind, [type, query, options, cb], this);
};

Store.prototype._qFind = function(type, query, options, cb, next) {
  type = type.toLowerCase().trim();
  ...
  var collection = this.collections[type];
  var results = this.AnywayCollection();
  ...
  if(options.select){
    ...
    for(var i = 0; i < options.select.length; i++) {
      queryOptions.fields[options.select] = 1;
    }
  }

  collection.find(query, queryOptions).toArray(function(err, docs) {
      ...
      for(var i = 0; i < docs.length; i++) {
        var result = model.create(docs[i]);
        result.id = docs[i]._id;
        result._saved = true;
        results.push(result);
      }
